...
from app.analysis import generate_and_persist_analysis_for_call
from app.analysis_provider import AnalysisProvider
from app.logging import log_event
from app.priority import calculate_and_persist_priority_for_call
    ...
    def _process_claimed(self, job: ClaimedJob) -> ProcessingResult:
        try:
            ...
            if self.analysis_provider is not None:
                analysis = generate_and_persist_analysis_for_call(
                    job.call_id,
                    self.database,
                    self.logger,
                    self.settings,
                ...
                priority = calculate_and_persist_priority_for_call(
                    job.call_id,
                    self.database,
                    self.logger,
                )
                log_event(
                    self.logger,
                    "processing_analysis_completed",
                    "Call analysis and Radar Priority were generated by the processing pipeline",
                    context={
                        "job_id": job.job_id,
                        "call_id": job.call_id,
                        "model_version": analysis.model_version,
                        "radar_priority": priority.score,
                    },
                )
            else:
                log_event(
                    self.logger,
                    "processing_analysis_skipped",
                    "No analysis provider was configured for this processing pipeline",
                    context={"job_id": job.job_id, "call_id": job.call_id},
                )
            with self.database.connect() as connection:
                row = self._load_job(connection, job.id)
                self._transition(
            ...
            reason = "analysis_failed" if error.status_code >= 500 else "processing_failed"
